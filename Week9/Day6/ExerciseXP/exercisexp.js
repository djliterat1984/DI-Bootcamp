...
// the sentence with the values replaced with the data above
...
const newArray = keys.map(key => [key, users[key]])
...
const newArray2 = keys.map(key => [key, users[key]*2])
...
//   }
// };
// Analyze the options below. Which constructor will successfully extend the Dog class? #2
//   // 1
...
object1.number = 4;
console.log(object2.number) // ==> 4  assigned value by reference, and modify all objects with the same reference
console.log(object3.number) // ==> 4 idem 
console.log(object4.number) // ==> 5 assigned the value
...
class Mammal extends Animal{
    ...
    sound ( sound ) {
        ...
        return `${sound} I'm a ${this.type}, named ${this.name}, and I'm ${this.color[0]} and ${this.color[1]}`
    }
}
